package github.viperthanks.shortlink.gateway.filters;

import com.alibaba.fastjson2.JSON;
import com.alibaba.fastjson2.JSONObject;
import github.viperthanks.shortlink.gateway.config.Config;
import github.viperthanks.shortlink.gateway.dto.GatewayErrorResult;
    ...
    public GatewayFilter apply(Config config) {
        return (exchange, chain) -> {
            ...
            if (!isPathInWhiteList(requestPath, requestMethod, config.getWhitePathList())) {
                ...
                Object userInfo;
                if (StringUtils.hasText(username) && StringUtils.hasText(token) && (userInfo = stringRedisTemplate.opsForHash().get("shortlink:login:" + username, token)) != null) {
